...
from mindspore.ops import expand_dims
...
def process(model=None, text="", tokenizer_=None, use_crf="", tag_to_index=None, vocab=""):
    ...
    for i in data:
        ...
        input_ids = expand_dims(Tensor(np.array(input_ids), mstype.int32), 0)
        input_mask = expand_dims(Tensor(np.array(input_mask), mstype.int32), 0)
        token_type_id = expand_dims(Tensor(np.array(token_type_id), mstype.int32), 0)
